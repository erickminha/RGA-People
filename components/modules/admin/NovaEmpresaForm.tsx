...
export default function NovaEmpresaForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const resultado = await criarEmpresa(formData);

    if (resultado.success) {
      router.push(`/c/${tenantSlug}/portal/admin/empresas`);
      router.refresh();
    } else {
      setMensagem(`❌ ${resultado.error || "Erro ao criar empresa"}`);
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 md:p-8 max-w-2xl mx-auto space-y-6">
      <header className="flex items-center gap-4">
        <Link
          href={`/c/${tenantSlug}/portal/admin/empresas`}
